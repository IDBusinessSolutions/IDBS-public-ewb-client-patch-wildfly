                        ...
                        case Protocol.CHANNEL_OPEN_REQUEST: {
                            ...
                            OUT: for (;;) {
                                ...
                                case Protocol.O_SERVICE_NAME: {
                                    ...
                                    log.debugf("CHANNEL OPEN: Service Type = %s", serviceType);
                                    break;
                                }
                                case Protocol.O_MAX_INBOUND_MSG_WINDOW_SIZE: {
                                    int senderMaxInboundWindowSize = ProtocolUtils.readInt(buffer);
                                    log.debugf("CHANNEL OPEN: received max inbound message size of %s", senderMaxInboundWindowSize);
                                    requestedOutboundWindow = Math.min(requestedOutboundWindow, senderMaxInboundWindowSize);
                                    break;
                                }
                                case Protocol.O_MAX_INBOUND_MSG_COUNT: {
                                    int senderMaxInboundMsgCount = ProtocolUtils.readUnsignedShort(buffer);
                                    log.debugf("CHANNEL OPEN: received max inbound message count of %s", senderMaxInboundMsgCount);
                                    requestedOutboundMessages = Math.min(requestedOutboundMessages, senderMaxInboundMsgCount);
                                    break;
                                }
                                case Protocol.O_MAX_OUTBOUND_MSG_WINDOW_SIZE: {
                                    int senderMaxOutboundWindowSize = ProtocolUtils.readInt(buffer);
                                    log.debugf("CHANNEL OPEN: received max outbound message window size of %s", senderMaxOutboundWindowSize);
                                    requestedInboundWindow = Math.min(requestedInboundWindow, senderMaxOutboundWindowSize);
                                    break;
                                }
                                case Protocol.O_MAX_OUTBOUND_MSG_COUNT: {
                                    int senderMaxOutboundMsgCount = ProtocolUtils.readUnsignedShort(buffer);
                                    log.debugf("CHANNEL OPEN: received max outbound message count of %s", senderMaxOutboundMsgCount);
                                    requestedInboundMessages = Math.min(requestedInboundMessages, senderMaxOutboundMsgCount);
                                    break;
                                }
                                case Protocol.O_MAX_INBOUND_MSG_SIZE: {
                                    long senderMaxInboundMessageSize = ProtocolUtils.readLong(buffer);
                                    log.debugf("CHANNEL OPEN: received max inbound message size of %s", senderMaxInboundMessageSize);
                                    requestedOutboundMessageSize = Math.min(requestedOutboundMessageSize, senderMaxInboundMessageSize);
                                    break;
                                }
                            ...
                            log.debugf("CHANNEL OPEN: Options TRANSMIT_WINDOW_SIZE = %s (default %s)", outboundWindowOptionValue, RemotingOptions.INCOMING_CHANNEL_DEFAULT_TRANSMIT_WINDOW_SIZE);
                            ...
                            log.debugf("CHANNEL OPEN: Options MAX_OUTBOUND_MESSAGES = %s (default %s)", outboundMessagesOptionValue, RemotingOptions.INCOMING_CHANNEL_DEFAULT_MAX_OUTBOUND_MESSAGES);
                            ...
                            log.debugf("CHANNEL OPEN: Options RECEIVE_WINDOW_SIZE = %s (default %s)", inboundWindowOptionValue, RemotingOptions.INCOMING_CHANNEL_DEFAULT_RECEIVE_WINDOW_SIZE);
                            ...
                            log.debugf("CHANNEL OPEN: Options MAX_INBOUND_MESSAGES = %s (default %s)", inboundMessagesOptionValue, RemotingOptions.DEFAULT_MAX_INBOUND_MESSAGES);
                            ...
                            log.debugf("CHANNEL OPEN: Options MAX_OUTBOUND_MESSAGE_SIZE = %s (default %s)", outboundMessageSizeOptionValue, RemotingOptions.DEFAULT_MAX_OUTBOUND_MESSAGE_SIZE);
                            ...
                            log.debugf("CHANNEL OPEN: Options MAX_INBOUND_MESSAGE_SIZE = %s (default %s)", inboundMessageSizeOptionValue, RemotingOptions.DEFAULT_MAX_INBOUND_MESSAGE_SIZE);
                            ...
                            log.debugf("CHANNEL OPEN: outboundWindowSize = %s", outboundWindow);
                            log.debugf("CHANNEL OPEN: outboundMessages = %s", outboundMessages);
                            log.debugf("CHANNEL OPEN: inboundWindow = %s", inboundWindow);
                            log.debugf("CHANNEL OPEN: inboundMessages = %s", inboundMessages);
                            log.debugf("CHANNEL OPEN: outboundMessageSize = %s", outboundMessageSize);
                            log.debugf("CHANNEL OPEN: inboundMessageSize = %s", inboundMessageSize);
