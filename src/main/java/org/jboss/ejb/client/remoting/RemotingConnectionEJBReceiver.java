...
public final class RemotingConnectionEJBReceiver extends EJBReceiver {
    ...
    private static final Logs logger = Logs.REMOTING;
    ...
    private void processInvocationInternal(EJBClientInvocationContext clientInvocationContext, EJBReceiverInvocationContext ejbReceiverInvocationContext) throws Exception {
        ...
        try {
            ...
            try {
                DataOutputStream dataOutputStream = wrapMessageOutputStream(clientInvocationContext, channelAssociation, messageOutputStream);

                CountingOutputStream byteCountingStream = null;
                if (logger.isDebugEnabled()) {

                    byteCountingStream = new CountingOutputStream(dataOutputStream);
                    dataOutputStream = new DataOutputStream(byteCountingStream);
                }
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Wrote " + byteCountingStream.getBytesWritten() + " bytes");
                    // Timings for transferring the data across the wire are not reliable at this point
                    // because the data is flushed at a later point.
                }
            } finally {
                try {
